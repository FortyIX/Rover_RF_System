...
int main(int argc, char *argv[]) {
	...
	int toBeSentCont[4] = {0,0,0,0};
		
	for(int i=0; i<=3;i++){
		
		  toBeSentCont[i] = (int)text[i];
	...
	for(int i=0; i<=3;i++){
		  ...
		  std::cout << toBeSentCont[i] <<std::endl;
	...
	for (;;)
	{
		...
		if (millis() - prev_millis_1s_timer >= interval)    // one second update timer
		{
    			...
    			Tx_fifo[3] = toBeSentCont[0] ;   //split 32-Bit timestamp to 4 byte array
    			Tx_fifo[4] = toBeSentCont[1] ;
    			Tx_fifo[5] = toBeSentCont[2] ;
    			Tx_fifo[6] = toBeSentCont[3] ;
